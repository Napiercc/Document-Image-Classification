...
def main(args):
    ...
    # 加载训练集
    train_dataset = datasets.ImageFolder(root='/tmp/pycharm_project_762/swin-transformer/data/train',transform=train_transform)
    ...
    # # 加载检查点
    # if os.path.exists('./run_results/best_model.pth'):  # 修改为检查点路径
    #     checkpoint = torch.load('./run_results/best_model.pth')
    #     net.load_state_dict(checkpoint)
    #     print("从检查点继续训练...")

    # # 假设已经训练了150个epoch，再训练100个epoch
    # start_epoch = 250
    # num_epochs = 350  # 总共训练250个epoch

    for epoch in range(100):
        epoch_start_time = time.time()  # 记录每个epoch的开始时间

        train_loss, lr = train_one_epoch(model=net, optim=optimizer, train_loader=train_loader, device=device, num_train=num_trainset)

        scheduler.step()

        test_acc, class_correct, class_total = evaluate(model=net, test_loader=test_loader, device=device, num_test=num_testset,class_correct=class_correct, class_total=class_total)
